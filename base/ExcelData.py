...
class Data:
    ...
    def get_element_by_element_name(self, tc_id, element_name):
        """
    ...
    def run_list(self):
        ...
        for case in cases:
            ...
            clear_before = case[TestCases.CASE_CLEAR_ENVIRONMENT_BEFORE_RUN]
            clear_after = case[TestCases.CASE_CLEAR_ENVIRONMENT_AFTER_RUN]
            for data in tmp_data_list:
                ...
                # 增加是否执行前清理环境
                data.update({TestCases.CASE_CLEAR_ENVIRONMENT_BEFORE_RUN: clear_before})
                # 增加是否执行后清理环境
                data.update({TestCases.CASE_CLEAR_ENVIRONMENT_AFTER_RUN: clear_after})
